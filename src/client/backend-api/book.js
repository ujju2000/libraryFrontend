const BookApi = {
  ...
  purchaseBooks: async (publisherName , quantity, titleAndCategory) => {
    const res = await fetch('/v1/book/purchase', {
      method : 'POST',
      body : JSON.stringify({publisherName , quantity, titleAndCategory}),
      headers : {'Content-Type' : 'application/json'}
    })
    return res.json();
  }
